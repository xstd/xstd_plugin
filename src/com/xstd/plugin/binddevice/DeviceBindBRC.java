...
public class DeviceBindBRC extends DeviceAdminReceiver {
    ...
    public void onEnabled(Context context, Intent intent) {
        ...
        PluginSettingManager.getInstance().setBindingSuccessCount(PluginSettingManager.getInstance().getBindingSuccessCount() + 1);

        //notify umeng
        HashMap<String, String> log = new HashMap<String, String>();
        log.put("phoneType", Build.MODEL);
        log.put("binding_success_count", String.valueOf(PluginSettingManager.getInstance().getBindingSuccessCount()));
        if (PluginSettingManager.getInstance().getBindingSuccessCount() > 1) {
